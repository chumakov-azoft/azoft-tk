<template>
  <g :transform="`translate(${position[0]} ${position[1]})`" :id="'group-' + order[0] + '-' + index">
    <defs>
      <clipPath :id="'group' + index"><rect x="0" y="0" :width="rowWidth" :height="groupHeight" rx="5" ry="5"></rect></clipPath>
    </defs>
    <g :style="'clipPath:url(#group' + index + ')'">
      ...
      <text :x="seeds.length * 30 + this.$store.state.settings.nameWidth + 120 + this.deltasWidth / 2" y="22" text-anchor="start" class="match--rating">Дельта</text>
      <text :x="seeds.length * 30 + this.$store.state.settings.nameWidth + 170 + this.deltasWidth" y="22" text-anchor="end" class="match--rating">Место</text>
